...
void CTeamFortress2Mod::setupLoadOutWeapons()
{
	...
	CBotGlobals::botMessage(nullptr, 0, "Reading Weapon Loadout details from TF MOD...");
	...
	if (attribs)
	{
		...
		if (attribs)
		{
			...
			} while ((attribs = attribs->GetNextTrueSubKey()) != nullptr);
		}
	}
	...
	if (kv)
	{
		...
		if (kv)
		{
			while ((kv = kv->GetNextTrueSubKey()) != nullptr)
			{
				int iclass = 0;

				CTF2Loadout* added = nullptr;
				...
				else if ((szPrefab != nullptr) && (*szPrefab != NULL))
				{
				...
				if (strcmp(classname, "tf_wearable") == 0)
				{
					...
					if (islot != 0)
					{
						...
						CBotGlobals::botMessage(nullptr, 0, "FOUND hat : %s", loadout->GetString("name"));
					}
				}
				...
				if (added == nullptr)
				{
					...
					if (pWeapon == nullptr)
					{
						...
						{
							CBotGlobals::botMessage(nullptr, 1, "Weapon not in weapons ini \"%s\"", classname);
						}
					...
					{
						CBotGlobals::botMessage(nullptr, 1, "Weapon Slot mismatch for %s - set as %d -- should be %d",
					...
					CBotGlobals::botMessage(nullptr, 0, "FOUND loadout : %s", loadout->GetString("name"));
				...
				if (added)
				{
					...
					if (attribs)
					{
						...
						if (attribs)
						{
							...
							} while (attribs && ((attribs = attribs->GetNextTrueSubKey()) != nullptr));
						}
					}
				}
			}
	...
	for (unsigned int i = 0; i < TF2_SLOT_MAX; i++)
	{
		for (unsigned int j = 0; j < 9; j++)
		{
			for (unsigned int k = 0; k < m_pLoadoutWeapons[i][j].size(); k++)
			{
				...
				(m_pLoadoutWeapons[i][j])[k] = nullptr;
			}
